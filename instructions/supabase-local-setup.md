...
_You can find your Project ID in the Supabase Dashboard settings._
...
_Note: If you run into a migration history mismatch, run `supabase migration repair --status reverted <migration_id>` as suggested by the error prompt before running `db pull` again._
...
_Important: Always use the `-f supabase/seed.sql` flag instead of `>` in Windows PowerShell to avoid UTF-16 encoding issues._
...
Step 1
$env:SUPABASE_DB_PASSWORD="Demon-Vape123"

Step 2
Repair remote migration history to match local. Should be something like this :

- supabase migration repair --status applied 20260316091500 (`EXAMPLE ONLY`)
